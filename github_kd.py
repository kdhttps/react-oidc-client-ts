...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        print "GitHub auth: authenticate called for step %s" % str(step)
        ...
        if step == 1:
            ...
            identity.setWorkingParameter("github_username", userInfo["login"])
            identity.setWorkingParameter("github_access_token", tokenResponse["access_token"])

            print "GitHub: Successfully authenticated"
            print "Github auth: username %s" % userInfo["login"]
            print "Github auth: github access token %s" % tokenResponse["access_token"]
            print "Github auth: github username %s" % identity.getWorkingParameter("github_username")
            ...
            if repositoryPath is None:
                print "GitHub. GitHub repository path is blank"
                return False
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if step == 1:
            ...
            user = authenticationService.getAuthenticatedUser()
            if user == None:
                print "GitHub. Authenticate for step 2. Failed to determine user name"
                return False
    ...
    def getExtraParametersForStep(self, configurationAttributes, step):
        ...
        return Arrays.asList("github_username", "github_access_token", "github_repository_path")
